...
import { Checkbox, Input, Select, SelectItem, Textarea, Tooltip, Image, Button, Accordion, AccordionItem, Link } from "@nextui-org/react";
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("/api/profile", {
        ...
        body: JSON.stringify({ ...data, City : data.City?.replace(/ /g, '_') ,id: session?.data?.user.id }),
      });
      // Handle the response
      if (response.ok) {
        ...
        <DefaultLayout>
          ...
          <form onSubmit={handleSubmit} >
            ...
            <div className='font-mont mt-3  px-3 py-5 rounded-xl border border-gray-500 shadow-gray-600 -m-1 my-auto'>
              <h1 className={title({ size: "sm", color: "violet" })}>Basic Details <Link href='/otp'><EditIcon className="text-default-500 inline w-7 h-7" /></Link></h1>
